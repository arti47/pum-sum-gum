...
| Character sheet as a *file* | **Files** (§4.2 `files.js`) — the PDF from the player's own RPG, filed and handed back. This is not the omitted §9.2 character sheet: the app stores the document and never reads it, so no stat is modelled and none is invented |
...
| Where it is stored | **The record in `localStorage`; files in IndexedDB.** Amended 2026-09-20, and the reason is the record's own health: `localStorage` is a ~5MB string store, base64 inflates a blob by a third, and one battle map would therefore fill it and take the campaign down with it — a quota error on save is how a solo player loses a year of play. So the campaign stays what it was (one readable JSON paste, exportable in one copy) and the bytes of maps, portraits, character-sheet PDFs and voice notes live beside it in IndexedDB, referenced by id. Still no backend, still nothing sent anywhere. The visible cost is that `Export JSON` cannot carry files, so it says so and `Export everything` is offered beside it |
...
| `data-guidance.js` | The books' procedural framing: play states, flowchart, beat triggers, advice — plus the app's own `NEW_TO_SOLO`, `FIRST_BEAT_COACH`, `SESSION_STAGES` and `ENDING_PROMPTS`, which teach what neither book stops to explain |
...
| `.claude/agents/` | `solo-rpg-playtester.md` — the playtester: plays a session end to end and reports where play stalls, not whether the code is right |
| `.playtest/brief.md` | What the app says the game is, written from the app before playing — resolution, the loop, clocks, the oracle, the record, ending |
| `tests/` | Harnesses (`harness`, `smoke`, `interaction`, `audit-modals`, `audit-deep`, `audit-novice`, `audit-reach`, `audit-guide`, `audit-functions`, `audit-hostile`, `deadcode`), the playtest driver and its AUDIT run (`playtest`, `playtest-audit`, §11.1 H), probes (`probe-layout`, `probe-flow`, `probe-firstrun`), `cycle.mjs` — the stopping-rule runner (§7.2) — seed fixtures, and `tests/tools/` — the extraction scripts, the two guide generators (`gen-tutorial`, `gen-pdf`), and `harvest-jargon` (§7.3) |
...
| `store.js` | Campaign library, plot sheets, nodes, cast, journal, export/import, undo stack, `transact()` — one player action is one undo, `prefer()`/`markBeat()` — writes that must not consume it |
...
| `forge.js` | The generators — GUM's and the player's. The Forge screen (a section of More, with its own second-level nav), the plot seed, the grand oracle, the inspiration block mounted inside every text field it can serve, and **My tables**: random tables the player typed in, which are app-level and deliberately not behind the GUM toggle |
| `media.js` | The blob store: IndexedDB, object URLs and their revocation, sizes, base64 for the bundle export, and the sweep that deletes what the record no longer points at. Degrades to a sentence rather than throwing when a browser refuses it |
| `files.js` | The Files screen (four drawers), the picker, voice recording, and the three places a file attaches: a journal entry, a cast portrait, a protagonist's character sheet |
...
| `coach.js` | The session coach: derives which of ten stages the player is in and renders one card — where you are, the one next thing, and the button. Owns the ending flow |
...
umState
  ...
  games[ {
    ...
    scopes[ {                              // one per plot sheet in play
      ...
      lastBeat: { ...the whole rolled beat, open } | null   // survives a reload (§8, 2026-09-08)
    } ],
    protagonists[ { id, name, notes, sheetId|null } ],      // sheetId: a PDF, not stats (§1.0)
    cast[ { id, kind:'character'|'location', name, notes, portraitId|null,
            traits[{table,label,text,roll}] } ],
    journal[ { id, ts, kind, title, dice[], detail, note, scopeId, sceneId, linkedTo,
               attachments[fileId] } ],
    files[ { id, name, type, form:'image'|'audio'|'pdf'|'file', size, addedAt,
             tag:'map'|'sheet'|'portrait'|'note' } ]   // METADATA ONLY — bytes in IndexedDB
  } ]
  tables[ { id, name, die, rows[], createdAt } ]   // the player's own, app-level not per-game

IndexedDB `umMedia` / store `files`: { id, blob }
  The only non-JSON store. `derived.referencedFileIds()` is the list of ids the
  record still points at, and `media.sweep()` deletes everything else — so that
  function must stay exhaustive as fields are added, or a file vanishes while
  something still shows it.
  `form` is deliberately not called `kind`: the harness reads every `kind: "..."`
  in `src/` as a journal kind needing a filter, and that guard is worth the word.
```
...
| A beat waits until it is judged, however long that takes | Gate | `scope.lastBeat` | `sheet.persistBeat`/`rehydrateBeat`, `store.markBeat` | The beat card, rebuilt on boot | `the beat is still there after reopening, and can be confirmed` |
...
| Custom Random Prompt column | Permission | `scope.customPrompts` | `store.setCustomPrompts`, `derived.promptColumn` | Track → Customize → Edit the prompt column | `a custom column survives a save and a reload` |
...
| A rolled result reaches the field it is FOR | Lookup | — | `forge.keepDialog` | Keep it → 14 destinations: node lists, the game's universe/tone/inspiration, the scope's mission/starting point/notes, a protagonist, the cast, the journal | `every destination writes, and appends rather than replaces` |
| A roll taken before the game exists is carried into prep | Permission | — | `startWizard(after, seed)` | Keep it → Prepare a game with this → a per-field strip on each wizard step | `the carried text reaches the field the player picks` |
...
| Files are the player's, not the app's | *the app's own* | `game.files`, IndexedDB | `media.*`, `files.js` | Play → Files; Portrait; Character sheet; Attach a file | `a file added through the app renders, survives a reload, and its metadata matches the blob` |
| A table the books never printed | *the app's own* | `state.tables` | `store.addTable`, `forge.rollTable` | More → My tables | `a pasted numbered list loses its numbering and rolls its own die` |
...
- [x] **Phase 12 — the things it stored no bytes for**: files (maps, character-sheet PDFs,
      portraits, voice notes) in IndexedDB beside the JSON record, and the player's own random
      tables. Asked for by comparing the app against a rival's feature list; three of the four
      gaps turned out to be one gap — the app stored no binary at all. See §8.
...
```sh
npm run cycle          # all fourteen passes, in order, never stopping at the first failure;
...
- **"Does it survive input it did not write?"** `npm run audit:hostile` is the one lens aimed
  at the two doors that take foreign data — every text field, and Import JSON. It reports what
  it covered *and what it deliberately does not*, so a clean run is a bounded claim: it says
  nothing about storage quota, older schema versions, or performance.
...
|---|---|---|---|
| 2026-09-20 | **Four features the app did not have, and three of them were one gap.** Measured against a rival's feature list rather than against the books: *journal with image and voice*, *manage maps and PDFs*, *character sheets*, *random tables*. The first three were the same absence — **the app stored no bytes at all.** §1.1's "localStorage only" is amended rather than abandoned: the record stays exactly what it was, one readable JSON paste, and the bytes move to IndexedDB (`src/media.js`) with `umState` keeping metadata only. That division is the point — base64 inflates a blob by a third into a ~5MB string store, so one battle map in `localStorage` would have taken the campaign down with it. New `src/files.js`: a **Files** section on the Play tab with four drawers (maps · character sheets · portraits · other), one reused hidden picker, voice recording through `MediaRecorder` that asks for the microphone when Start is pressed rather than when the dialog opens and says so in place when there is none, and three attach points — an entry's own **Attach a file**, a cast **Portrait**, a protagonist's **Character sheet**. That last is deliberately *not* the omitted §9.2 character sheet: the app files the PDF from the player's own RPG and hands it back, models no stat, and never reads it. `Export JSON` cannot carry files, so it now says how many it is leaving behind and **Export everything** is offered beside it, warning about the size before it builds one; Import takes either. Everything degrades to a sentence when a browser refuses IndexedDB. Fourth feature, and the app's own rather than any book's: **My tables** — paste a numbered list, the `1.`/`2)`/`3 -` numbering is stripped, and it rolls with its own die, journals under a new `table` kind with its filter, and reaches the same fourteen "Keep it →" destinations GUM does. Deliberately **not** behind the GUM toggle: switching GUM off says you do not own that book and says nothing about a table you typed in yourself. Two passes caught what the feature work missed — deadcode found five exports nothing imported (now module-private), and the function audit found the new screens unreachable because its journeys never visited them; three journeys added, and the first of those re-seeded mid-journey and wiped the file it had just added, reporting that file's own controls as unreachable. `goTo()` navigates without touching storage. Five functions are genuinely unclickable and now say why. | unit 1,875 · smoke 500 (new: a file added through the app's own picker, rendered, survived a reload, and removed from both stores at once; a pasted table stripped, rolled, journalled, and still there with GUM switched off) · both guards watched failing · functions 345/356 reachable by clicking, 11 expected with reasons · `npm run cycle`, fourteen passes · guide, page and PDF regenerated (32 pages) | v41 |
| 2026-09-08 | **The readable export was a log, and it is the one file that outlives the app.** Reported from play after the second session: 263 entries as a flat bullet list, each led by the internal kind key (`yesno:`, `gum:`, `prep:`), node lists printed under their storage keys (`custom1:` for a list the player had named "The eleven weights"), the player's own paragraphs crammed into a bullet's title line, and continuation lines indented four spaces — which most Markdown renderers draw as a **code block**, so the prose came out in monospace inside a grey box. Rewritten as a document: front matter, protagonists, the cast with their SUM traits, a list of the storylines, then one section per scope carrying its sheet, track state, mission, starting point and node lists **under the names the player gave them** — and beneath that the play itself, cut into `### Scene N` on the scene-opened and scene-closed entries, with the epilogue always last in its storyline rather than wherever it happened to be written. The reader can tell the machine from the author at a glance: a roll is one block led by its bold name with its dice in code ticks (a die the Rule of Bias dropped is marked so), the player's own writing is a plain paragraph, and a note written about a roll is quoted beneath it. Track crossings and node invocations collapse to a one-line italic. Nothing in `src/` restates a category name: `derived.categoryName()` already knows the player's own. | smoke 477 (new: the export driven through the app, asserting it names the storylines, cuts into scenes, uses category names rather than storage keys, shows dice, and indents nothing into a code block) · guard watched failing on four assertions · `npm run cycle`, fourteen passes · guide, page and PDF regenerated (31 pages) | v40 |
| 2026-09-08 | **Played a second story, exhaustively, and a whole Permission was being erased by the act of saving it.** Four scopes on four sheet shapes (Journey 20 boxes · Customized, track drawn by hand · Dungeon 7 · Sandbox, trackless), 255 journal entries, every scope ended with an epilogue. **F-74: a customised Random Prompt column was emptied by the first reload.** A column entry is an object — `{label, node}` or `{label, event}` — and `normalize()` ran each one through `str()`, which returns `""` for anything that is not a string. The array was still ten long, so `rules.promptAt` preferred it over the sheet's own column and `column[roll-1] || null` was null for **every** face: each random prompt came back "(empty prompt slot)", silently, for good, with the Customize dialog still showing ten rows. On the Customized sheet that is the only prompt column there is, so it is a stall — and the track still offered "Confirm — cross a box" for a beat that said nothing. It was introduced by the cycle-8 hostile pass (v27), which made `normalize()` total by applying `obj()`/`str()` "to every raw field"; no pass caught it in twelve days because the unit test set a column and rolled it **in memory**, never through a save, and the browser passes press Customize but never reload and then roll. `derived.promptColumn()` now normalizes each entry by shape and drops a column that is not ten usable entries **whole**, so an already-damaged save falls back to the sheet's printed column rather than half-working — verified on this game's own broken state. **F-75: on the Scene tab the coach's own button did nothing.** `coachStrip()` (F-71) put the card on `scene/arc`, where `actionFor`'s three scene stages still ran `go("scene","arc")` — navigating to the screen the strip is rendered on. Pressing **"Open a scene"** there left `openScene` null and moved nothing; the controls that work are further down the same screen. Fixed the way F-67 was, with a `#scene-controls` anchor and scroll-or-navigate, and guarded by asserting `actionFor` never calls `go()` for a screen that carries the strip. Four more findings reported and not fixed, in `docs/AUDIT.md`: a biased Yes/No left unpicked is stranded in the record with no route back; the disruption die is rolled and fired and never journalled; the empty "Choose from …" dialog still offers only Cancel; and that dialog calls a player-named list "My list". The driver gained three things this session needed and it did not have — `read` (block-per-line text, since `state` truncates at 160 chars), `set` (the `<select>` and range controls), and `"Card > Label"` scoping for the ten identically-labelled sheet cards and the two-field dialogs. | unit 1,860 (new: the column round-tripped through `normalize()`, and a malformed column dropped whole; plus a source guard on the coach's actions) · both guards watched failing · `npm run cycle`, fourteen passes · both fixes verified in the running app, including recovery of the already-broken save | v39 |
| 2026-09-08 | **Played a session with the playtester, and the beat did not survive closing the app.** F-73, the one class of finding no structural pass can reach: the beat card was module state (`sheet.js`'s `openBeat`) while the *fact* of a beat being on the table was stored (`scope.lastBeat.open`). So a reload — or a phone locking, or a tab closing between two sittings — left the app asserting something it could no longer act on: the session coach says **"A beat is on the table"** and offers **"Go to the beat"**, and the beat controls it scrolls to offer only *Modified proposal · Random prompt*. The rolled beat is already in the journal, the box is never crossed, and the only way on is to roll a different one. It cost four beats in one played session before it was diagnosed. Fixed by storing the whole rolled beat beside the flag and rebuilding the card from it on boot; node choices made inside a beat persist the same way. `store.setLastBeat()` is replaced by `store.markBeat()`, which emits without snapshotting: the move was the roll, and confirming already runs inside `transact`, so re-recording must not consume a capped undo stack — choosing a node inside one beat would otherwise evict a real action, the F-46 defect in a new place. The function audit caught the leftover: with the roll sites moved, `setLastBeat` was reachable by no sequence of clicks. Old states keep only `key`/`text` and fall back to the chooser rather than drawing a half-card. Guard watched failing, and made to report the missing control by name rather than throw a locator timeout. | smoke 470 (new: roll a beat, boot a second time from exactly what the first left in `localStorage`, confirm it there) · `npm run cycle`, fourteen passes | v38 |
| 2026-09-08 | **A playtester, and the first sessions it played.** Every existing pass asks a structural question — is this control reachable, is this rule implemented, does this function run — and all of them stay green on an app nobody can play a session with. `.claude/agents/solo-rpg-playtester.md` (supplied) is installed, and with it `tests/playtest.mjs`: a driver to its contract — `new · state · screen · do · choose · type · write · journal`, verbs chaining in one invocation because a dialog cannot survive a reload, the campaign persisted in `.playtest/state.json`, controls found **by visible label** because a control a player cannot name from the screen is itself a finding. `--seed` stubs `crypto.getRandomValues`, not `Math.random`, which this app never uses; branch choices come from a **second, separate** PRNG, so answering a dialog does not shift the dice and three seeds are three sessions rather than one in costume. `tests/playtest-audit.mjs` plays the spine on seeds 7, 23 and 101: prep → scene → four oracle-and-beat exchanges → intervention → closure → the record in the player's own words → end the scope. **All three reached an ending; 0 stalls, 0 console errors.** One finding, on two seeds: the empty *Choose from …* dialog says "Add a new node instead" and offers only Cancel — a described capability with no control. Reported, not fixed: the playbook reports by default. Three of its own traps were hit while building it — the confirm button is "End the scope" not the card's "End this scope", so matching the opener's words inside the dialog it opened pressed nothing and read as success (the first run claimed an ending the record showed had not happened; the verdict now reads `scope.closedAt`); a dialog left open turned every later beat into a phantom stall; and once the session coach landed, a spine that assumed "Open a scene" stalled on all three seeds until it learned to **follow the coach's own next move**, which is what "press only what the app offers" actually means. | playtest audit: 3 seeds, 0 stalls, 0 console errors, every seed ended its scope and wrote its own words into the journal · unit 1,845 · dead-data clean · no shipped file changed, so no cache bump | v37 |
| 2026-08-27 | **The same report, in the same words, a second time — the coach named buttons where it needed to name moves.** Its sustaining-play stage read "This is most of the game: you play your protagonists and narrate what happens. Next: keep playing" — a description of solo roleplaying, not an instruction anyone can follow, and "narrate what happens" is the very thing being asked about. F-70: every stage now carries literal numbered `steps`, and an `example` in the player's own voice wherever "say something" is the instruction. The scene stage is the loop itself: say two sentences → ask yourself whether you know what happens next → if you do, say it and go round again (**you do not need to roll anything** — a beginner's commonest failure is rolling when they already know) → if you do not, ask an oracle; if you are stuck rather than uncertain, call a beat. F-71: the coach existed on one screen out of twenty, and playing happens on the Scene and Oracles tabs as much as on Play — `coachStrip()` is the short form and follows the player onto both. F-72: mounted above each Scene screen's heading it made four routes lead with a control instead of a sentence; the novice audit caught it in the same run and it now sits under each screen's note. | unit 1,870 · the scene-open stage read back from the running app · novice audit 0 findings after the placement fix · `npm run cycle`, fourteen passes | v37 |
| 2026-08-27 | **Reported from play: "I still don't know how to start playing, sustain play, and end the game well — I want to play straight from the app without reading the manual."** Every teaching surface this app had was a **document**: a walkthrough, a glossary, a rules library, per-screen notes. All of them ask the player to go and read, and none answers "what do I do right now". F-64: new `src/coach.js` and `SESSION_STAGES` — the app already knows exactly where the player is, so it now derives one of ten stages from live state and renders a single card at the top of the Play tab: what is true now, the one next thing, and the button. No mode to enable, nothing to finish, nothing remembered — it is derived, so it cannot go stale. It replaces the old `whatNowCard`, which said one true thing about the middle of a session, nothing about starting or ending, and sat below the beat controls where a player asking "what now" had already given up. F-65: **ending was the third of the arc with no surface at all.** A full track said "the track is full" and offered "Start another plot sheet" — a storyline that just stops is the commonest way a solo game feels unfinished. `endingDialog` asks four questions the books do not (recorded as the app's own, like ruling A8's absent safety tools), writes an `ending` journal entry, marks the scope finished, and moves the coach to a "finished, and written down" stage that offers the next storyline. `ending` joins the journal filters, because a kind with no filter cannot be found again. F-66: "open your **first** scene" was tested by looking for a SUM scene entry, so a player who had rolled fourteen beats and crossed two boxes without ever opening a formal scene was still told to open their first — now the test is whether anything has happened in this scope at all. Plus three defects the passes caught in the new code: F-67 three coach actions navigated to the screen they were already on (the deep audit reported it on all ten sheets); F-68 dropping the old card took the journal route with it and renamed another, one destination with two labels; F-69 "have you played here yet" was tested wrong twice — a SUM scene entry missed players who never open a formal scene, and *any* journal entry counted prep bookkeeping, so writing a plot node in the wizard made a brand-new game report as mid-session. | unit 1,843 · all ten stages walked in a browser and read back · the ending flow walked end to end: dialog → journal entry → stage advances · first-run probe: 20 taps, every step offered in place · guide/page/PDF regenerated · `npm run cycle`, fourteen passes · merged with the guide's new Part 1, which answers the same report in prose where the coach answers it on screen | v36 |
| 2026-08-27 | **Reported from play a third time: the rolls do not help fill the scope name.** F-62, visible in the screenshot: the same row offered **twice** — `scope-name` mapped to one table, `pickTables` pushes it three times, and three d20 rolls on a 20-row table collide ~14% of the time. `rollDistinct()` re-rolls a repeat, bounded. F-63: a scope name cannot be rolled at all. The field wants "Find out who burned **the caravan**" — the caravan is the player's own noun, and GUM generates fiction, not the nouns of a game it has never seen. The mapping had already been narrowed twice (six seed tables → the mission table alone, F-45); both passes judged the rows by **shape** — goal-shaped, right length — and both missed that shape was never the problem. `scope-name` joins `INSPIRE_ABSENT` beside `game-title`, which is the identical defect fixed the same way one round earlier (F-44), and the entry gives the advice that works: write the mission first, then lift the name out of it. Ten fields now state a reason for offering no roll. Recorded as a pattern: the test for a mapping is not "is this the right shape?" but "could this row mention the thing the player is naming?". | unit 1,847 · step 2 rendered and read back: name field has no roller, mission offers 6 distinct rows · guide/page/PDF regenerated · `npm run cycle`, fourteen passes | v33 |
| 2026-08-27 | **Reported from play, both rollers circled: the rolls do not fit the content needed.** F-58: the Mission field's hint asks for "a paragraph, not a title", and its mapped set is exactly GUM's six-table plot seed — but the block rolled **three of the six** and joined them with a middot, so a hook, a motivation and a mission from three different questions arrived as one line of fragment soup. Three of six is neither the book's method (all six, read together, GUM p.3) nor a paragraph. A multiline field now rolls its whole set, and "Use all 6" writes labelled lines — a skeleton to write over. F-59: the offers were bare sentences with their table names in one small line above in matching order, so three results from three different tables read as three unrelated ones; each offer now carries its table and roll. F-60: the block was called "Stuck? Roll three words" and not one row in any mapped table is a word — now "Roll some ideas". F-61: `appendToField` already knew a textarea joins on newlines and an input on middots; `keepInspiration` flattened its parts before the append ever saw them. Recorded and not changed: the scope-name field's offers are goals in four to six words, which is what that field asks for (F-45); the dangling pronouns are the book's own text and adapting them is the intended use. | unit 1,867 · step 2 rendered, both blocks opened, and what "Use all" writes read back out of the field · guide audit 0 drift, guide/page/PDF regenerated · `npm run cycle`, fourteen passes | v32 |
| 2026-08-27 | **Reported from play, with both fields circled: "Plot scope name" and "Mission" are hard to tell apart.** Three things on that screen pushed them together. F-54: the card's own intro read "a plot scope is one defined **mission**, task or goal" and was followed by two fields, one called Mission — the screen defined the scope as a mission and then asked for both. Now "one storyline with an end in sight", plus an explicit contrast line: the name is a short label you see at the top of every screen, the mission is the paragraph underneath. F-55: the name field was placeheld with a real example ("Find out who burned the caravan") and the mission field with an *instruction* ("A pitch for the situation you start in…") — one field showed its shape, the other only asserted it, and on a dark screen the instruction looked like content already in the box. Mission now carries a real example, and both carry a hint naming the difference: "a handle, not a description — two to six words" against "a paragraph, not a title". F-57: one stored field with two labels — "Plot scope name" in the wizard, "Scope name" in both dialogs — the defect the 2026-08-21 row guards against, in a place the guard does not look. F-56 recorded and deliberately not changed: the name field's roller is GUM's mission table, which is right by shape (F-45) and reads oddly only because the fields were indistinct. | unit 1,872 · step 2 rendered and read back · guide audit 0 drift, guide/page/PDF regenerated · `npm run cycle`, fourteen passes | v31 |
| 2026-08-27 | **Reported from play: "I still don't know what the rolls from the Forge are for."** The previous two rounds fixed where a result *goes*; neither said what it is *for*. F-47: the prep wizard never mentioned the Forge at all — five empty fields and no route to the machine that invents a starting situation; now a "Do not know yet?" card on steps 1 and 2, with the draft preserved while you go and look. F-48: the Forge's note listed what the six tables *are* ("a hook, a motivation, a mission…"), which describes the machine and never says what you get out of it; now purpose first. F-49: no worked example — six abstract table names and nothing showing what a rolled seed looks like as a game; now `GUM_SEED_EXAMPLE`, a real roll at real faces, read as one situation, **and where each of the six lines ended up**. F-50: keeping a result mid-prep called `startWizard()`, which resets the draft — the label said "Prepare a game" to someone already preparing one, and the click would have discarded everything typed; now `carryIntoWizard()` attaches to the existing draft and the label reads "Take it back to prep". Plus three tooling defects the function audit surfaced by refusing to pass: coverage read only the first record per script URL (a reload made covered functions look unreachable); the audit's own `seed()` left `main.js`'s cached context stale so the next mutation wiped the wizard draft mid-journey; and a `replace(…, 1)` put debug instrumentation in the wrong journey. | unit 1,858 · the control-completeness guard caught all three new controls before they shipped undocumented · guide, page and PDF regenerated · walked by hand: type a title → Forge → roll → carry back → title survives · `npm run cycle`, fourteen passes | v30 |
| 2026-08-27 | **Reported from play: the prep wizard's inspiration rolls did not match the fields.** Three mismatches, all one defect — a mapping written from a table's *name* rather than from its *rows*. F-44: "Name this game" rolled the grand oracle's adjective and subject tables, on the premise (stated in `data-gum.js`'s own comment) that those are "word-shaped" and readable as a title. **Not one of their 200 rows is a single word** — every one is a cluster of four to eight synonyms, so the offer was to paste a thesaurus entry into the title field. This is the same defect the 2026-08-17 row claims to have fixed: that pass diagnosed the class correctly and then built the fix on the same unchecked premise. `game-title` now sits in `INSPIRE_ABSENT` beside `game-universe`, saying so and pointing at the world and tone fields instead. F-45: "Plot scope name" wants a goal in a line ("Find out who burned the caravan") and rolled a plot hook (an event) and a motivation (a reason) alongside the mission; now `["mission"]` alone, three rolls within one table being GUM p.3's own method, which yields three candidate goals. F-46: "World, tone and theme" led with a synonym cluster where GUM has two tables answering the question directly; now location archetype, background problem, society. | unit 1,853 · every field's offers read back out of the running wizard and checked against what the field asks for · `npm run cycle`, fourteen passes | v29 |
| 2026-08-27 | **Reported from play: a Forge result could not reach the thing it was for.** GUM is a prep tool by the books' own division of labour, and its six-table plot seed is *a hook, a motivation, a **mission**, a first lead, a caveat, an opposition* — yet "Keep it →" offered only plot-node lists, the cast and the journal. Not one of those six lines could reach the plot sheet's Mission, the game's universe, its tone, its inspiration, the starting point, the game notes, or a protagonist. Worse, the moment a plot seed is *most* useful is before a game exists — and there the dialog said "there is nowhere to keep this" and dropped the roll on the floor, sending the player to a blank wizard to retype six lines by hand. Fixed in both directions: 14 destinations when a game is open (the six setting/scope fields folded, since node lists are the commoner answer in play), all of them **appending rather than replacing**; and `startWizard(after, seed)` carries a roll into prep, where a "Rolled in the Forge" strip offers it against each field on the step that owns it, with Dismiss. | unit 1,861 · the control-completeness guard caught both new controls before they shipped undocumented · guide, page and PDF regenerated · both directions walked in a real browser: no game → carry → field, and game open → all 14 destinations listed and the mission write verified | v28 |
| 2026-08-27 | **Cycle 8 — LENS: hostile input.** Run from the audit-prompts playbook: converge, then widen with one named lens. New `tests/audit-hostile.mjs` (832 checks) points 16 payloads and 40 malformed states at the two doors that take data the app did not write — every text field, and Import JSON. **21 findings.** F-38: `normalize()` was not total. It threw on `null` and on a `null` element inside games, scopes, protagonists, cast or journal — and since `store.load()` is `JSON.parse(localStorage)` → `normalize()`, any of those meant the app **would not boot at all**: a blank screen and a campaign that looks deleted. A seventh shape put the literal text "[object Object]" on Home, a *type* nobody checked rather than a null leaking through. Fixed with `obj()`/`str()` helpers applied to every raw field, so it is now total by construction. F-39: one long word destroyed the layout — a 4,000-character unbroken token overflowed 390px by **83,214px**, and something as ordinary as a pasted link by 58-290px. Fixed with `overflow-wrap: anywhere` on the five containers holding player text, tables and `<pre>` exempted to scroll. **Injection found nothing** — `el()`'s `textContent` discipline holds and its `html:` escape hatch has no call site anywhere — and neither did unicode, round-trip or import rejection. The clean run prints its coverage boundary: this lens says nothing about storage quota, older schema versions, or performance. Both guards watched failing. | `npm run cycle`, fourteen passes · unit 1,861 (19 hostile shapes through `normalize()` in the cheap pass) · hostile 832 checks, 0 findings | v27 |
...
| 2026-08-21 | **The guide taught the app and never taught the play.** Reported from play, after three passes that all came back clean: *"I still don't know how to start playing, sustaining play, and ending the game well."* Every control was reachable, correctly named and documented — and none of that answers what to say when the page is blank. The quick start is eleven steps of which-button; Part 2 walks every function; nothing anywhere covered the rhythm. New **Part 1 · How to actually play**, four sections and the guide's new opening: *Your first ten minutes* (invent a situation and a want in one sentence, three nodes not thirty, open in the middle of something, narrate two sentences and then do something physical, and only then roll — with the sentences to say, written out); *Keeping it going for an hour* (roll only when you don't know **and** the answer changes what happens next; the two ways a session dies — dice soup and drift — with the fix for each; when stuck, stop asking oracles and roll a prompt; a scene is five to fifteen minutes; cross a box only when something changed); *Ending a scene, a session, and a story* (end a scene on a change, end a session on a hook while you still want to continue, and spend the track's last third paying off rather than introducing — an ending answers the scope's question, costs something, and leaves exactly one thing open); and *If you are stuck right now*, six moves to read down. Parts 1-4 renumber to 2-5. | unit 1576 · guide audit 106 claims, 0 the app never says · dead-data clean · smoke 464 · novice audit 0 findings · guide, page and PDF regenerated (26 → 30 pages) · `more/tutorial` 2.9 → 3.1vh | v25 |
