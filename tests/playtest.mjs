...
for (let i = 0; i < argv.length; i += 1) {
  ...
  if (v === "--quiet") continue;
  if (["new", "state", "journal", "screen", "do", "choose", "type", "write", "controls"].includes(v)) {
    const takesArg = ["screen", "do", "choose", "type", "write"].includes(v);
...
const look = () => page.evaluate(() => {
  ...
  return {
    ...
    dialog: modal ? {
      ...
      fields: [...modal.querySelectorAll("input[type=text], textarea")].length,
    } : null,
...
async function press(label, where) {
  const hit = await page.evaluate(([want, sel]) => {
    const vis = (n) => n && n.offsetParent !== null;
    const txt = (n) => (n.textContent || "").replace(/\s+/g, " ").trim();
...
async function typeInto(text) {
  return page.evaluate((v) => {
    ...
    const f = [...box.querySelectorAll("input[type=text], textarea")].filter(vis)
      .find((n) => !n.value) || [...box.querySelectorAll("input[type=text], textarea")].filter(vis)[0];
    if (!f) return false;
    f.value = v;
